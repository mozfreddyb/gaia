  ...
  var PasscodeHelper = {
    ...
    setPasscode: function (newPass) {
      ...
      var digest = getFromSettings.then((values) => {
         ...
         */
        console.log('Stored iterations & algorithm count', values);
        ...
        return this._makeDigest(newPass, salt, iterations, algorithm)
          .then((digest) => {
            ...
            return lock.set(newSettings).then(() => { // TypeError: lock.set(...).then is not a function
              return digestUint8;
            });
          }, (error) => {
...
(function (exports) {
  ...
  var PasscodeHelper = {
    ...
    _makeDigest: function (pass, salt, iterations, algorithm) {
      console.log("Making digest", pass, salt, iterations, algorithm);
      var bytes = this._encode(pass);
      return crypto.subtle.importKey('raw', bytes, 'PBKDF2', false, [
        'deriveBits'
      ]).then((pwKey) => {
